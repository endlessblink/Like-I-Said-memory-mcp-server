...
// Initialize the task manager with race condition prevention
...
export async function getTaskManager() {
  ...
  if (taskManager && initializationAttempted) {
    return taskManager;
  }
  
  // Create initialization promise to prevent race conditions
  initializationPromise = initializeTaskManager();
...
async function initializeTaskManager() {
  ...
  console.error('[TaskManager] Initializing HybridTaskManager...');
  
  try {
    taskManager = new EnhancedHybridTaskManager();
    await taskManager.initialize();
    console.error('[TaskManager] HybridTaskManager initialized successfully');
    return taskManager;
  } catch (error) {
    console.error('[TaskManager] HybridTaskManager initialization failed:', error.message);
    // Reset state to allow retry
    taskManager = null;
    initializationAttempted = false;
    throw new Error(`TaskManager initialization failed: ${error.message}`);
  }
}

// Add verification method
export async function verifyTaskManagerInitialization() {
  try {
    const tm = await getTaskManager();
    if (!tm) {
      throw new Error('TaskManager is null after initialization');
    }
    
    // Test basic functionality
    if (typeof tm.verifyConnection === 'function') {
      await tm.verifyConnection();
    }
    
    return { status: 'success', message: 'TaskManager verified successfully' };
  } catch (error) {
    return { 
      status: 'error', 
      message: `TaskManager verification failed: ${error.message}` 
    };
  }
}
...
export const v3Tools = [
...

  {
    ...
    description: 'Create a complete project structure with stages and tasks in one operation. Reduces errors and token usage.',
    inputSchema: {
      "$schema": "https://json-schema.org/draft-07/schema",
      type: 'object',
      properties: {
        ...
        stages: {
          type: 'array',
          description: 'List of stages to create under the project',
          items: {
            type: 'object',
            properties: {
              ...
              tasks: {
                type: 'array',
                description: 'List of tasks to create under this stage',
                items: {
                  type: 'object',
                  properties: {
...

  {
    ...
    description: 'Validate if a hierarchy operation would succeed. Use this to check before creating tasks/stages to avoid errors.',
    inputSchema: {
      "$schema": "https://json-schema.org/draft-07/schema",
      type: 'object',
      properties: {
        operation: {
          type: 'string',
          enum: ['create_stage', 'create_task', 'create_subtask'],
          description: 'The operation you want to perform'
        },
        parent_id: {
          type: 'string',
          description: 'ID of the parent task/project (if applicable)'
        },
        project_name: {
          type: 'string',
          description: 'Project name (alternative to parent_id)'
        }
      },
      required: ['operation'],
      "additionalProperties": false
...

  {
    ...
    description: 'Find an existing project or create it if it doesn\'t exist. Prevents duplicate projects and wasted tokens.',
    inputSchema: {
      "$schema": "https://json-schema.org/draft-07/schema",
      type: 'object',
      properties: {
        title: {
          type: 'string',
          description: 'Project name to find or create'
        },
        description: {
          type: 'string',
          description: 'Project description (used only if creating new project)'
        },
        priority: {
          type: 'string',
          enum: ['low', 'medium', 'high'],
          description: 'Priority (used only if creating new project)'
        },
        tags: {
          type: 'array',
          items: { type: 'string' },
          description: 'Tags (used only if creating new project)'
        }
      },
      required: ['title'],
      "additionalProperties": false
  ...
  switch (toolName) {
    ...
    case 'setup_project_structure': {
      try {
        ...
        const projectSlug = args.project.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        
        // Check if project exists
        const existing = await manager.findExistingProject(args.project.title);
        
        if (existing && existing.length > 0) {
          results.project = existing[0];
          results.errors.push(`Project "${args.project.title}" already exists - using existing project`);
        } else {
          // Create new project
          results.project = await manager.createTask({
            title: args.project.title,
            description: args.project.description || `Project: ${args.project.title}`,
            level: 'master',
            project: projectSlug,
            priority: args.project.priority || 'medium',
            status: 'todo',
            metadata: {
              created_via: 'mcp_setup_project_structure'
            }
          });
        }
        
        // Step 2: Create stages under the project
        if (args.stages && args.stages.length > 0) {
          for (const stage of args.stages) {
            try {
              const stageTask = await manager.createTask({
                title: stage.title,
                description: stage.description || `Stage: ${stage.title}`,
                level: 'epic',
                parent_id: results.project.id,
                project: results.project.project,
                priority: results.project.priority,
                status: 'todo',
                metadata: {
                  created_via: 'mcp_setup_project_structure'
                }
              });
              results.stages.push(stageTask);
              
              // Step 3: Create tasks under each stage
              if (stage.tasks && stage.tasks.length > 0) {
                for (const task of stage.tasks) {
                  try {
                    const taskItem = await manager.createTask({
                      title: task.title,
                      description: task.description || `Task: ${task.title}`,
                      level: 'task',
                      parent_id: stageTask.id,
                      project: results.project.project,
                      priority: task.priority || results.project.priority,
                      status: 'todo',
                      metadata: {
                        created_via: 'mcp_setup_project_structure'
                      }
                    });
                    results.tasks.push(taskItem);
                  } catch (error) {
                    results.errors.push(`Failed to create task "${task.title}": ${error.message}`);
                  }
                }
              }
            } catch (error) {
              results.errors.push(`Failed to create stage "${stage.title}": ${error.message}`);
            }
          }
        }
        
        // Build summary response
        let response = `✅ Project Structure Created\n\n`;
        response += `📁 Project: ${results.project.title}\n`;
        response += `   ID: ${results.project.id}\n`;
        response += `   Path: ${results.project.path}\n\n`;
        
        if (results.stages.length > 0) {
          response += `📂 Stages Created: ${results.stages.length}\n`;
          for (const stage of results.stages) {
            response += `   • ${stage.title} (${stage.path})\n`;
          }
          response += `\n`;
        }
        
        if (results.tasks.length > 0) {
          response += `📝 Tasks Created: ${results.tasks.length}\n`;
        ...
        response += `💡 Next Steps:\n`;
        response += `   • Use view_project("${results.project.id}") to see the full structure\n`;
        response += `   • Use create_subtask to add subtasks to any task\n`;
        response += `   • Use update_hierarchical_task to modify any item`;
        ...
        return {
          content: [{
            type: 'text',
            text: `❌ Failed to setup project structure: ${error.message}`
          }],
          isError: true
        };
      }
    ...
    case 'validate_hierarchy': {
      try {
        ...
        if (args.parent_id) {
          parent = await manager.getTask(args.parent_id);
          if (!parent) {
            return {
              content: [{
                type: 'text',
                text: `❌ Validation Failed: Parent with ID '${args.parent_id}' not found.\n\n` +
                      `💡 Suggestions:\n` +
                      `1. Use find_project to search for existing projects\n` +
                      `2. Use find_or_create_project to ensure the project exists\n` +
                      `3. Check the ID is correct using view_project`
              }],
              isError: false
            };
          }
          parentInfo = `Parent: ${parent.title} (Level: ${parent.level})`;
        } else if (args.project_name) {
          // Try to find project by name
          const projects = await manager.findExistingProject(args.project_name);
          if (projects && projects.length > 0) {
            parent = projects[0];
            parentInfo = `Project: ${parent.title} (Level: ${parent.level})`;
          } else {
            return {
              content: [{
                type: 'text',
                text: `❌ Validation Failed: Project '${args.project_name}' not found.\n\n` +
                      `💡 Suggestions:\n` +
                      `1. Use find_or_create_project("${args.project_name}") to create it\n` +
                      `2. Use find_project to search for similar projects`
              }],
              isError: false
            };
          }
        ...
        switch (args.operation) {
          case 'create_stage':
            if (!parent) {
              validation = '❌ Cannot create stage without a parent project';
            } else if (parent.level !== 'master') {
              validation = `❌ Cannot create stage under ${parent.level}. Stages can only be added to projects (master level).`;
            } else {
              canProceed = true;
              validation = `✅ Valid: Can create stage under project "${parent.title}"`;
            }
            break;
            
          case 'create_task':
            if (!parent) {
              validation = '❌ Cannot create task without a parent';
            } else if (parent.level === 'master') {
              validation = '❌ Cannot create task directly under project. Create a stage first.';
            } else if (parent.level === 'epic') {
              canProceed = true;
              validation = `✅ Valid: Can create task under stage "${parent.title}"`;
            } else if (parent.level === 'task') {
              validation = '❌ Cannot create task under another task. Use create_subtask instead.';
            } else {
              validation = `❌ Invalid parent level: ${parent.level}`;
            }
            break;
            
          case 'create_subtask':
            if (!parent) {
              validation = '❌ Cannot create subtask without a parent task';
            } else if (parent.level !== 'task') {
              validation = `❌ Cannot create subtask under ${parent.level}. Subtasks can only be added to tasks.`;
            } else {
              canProceed = true;
              validation = `✅ Valid: Can create subtask under task "${parent.title}"`;
            }
            break;
        }
        
        // Build response with recommendations
        let response = `Validation for: ${args.operation}\n`;
        if (parentInfo) response += `${parentInfo}\n`;
        response += `\n${validation}\n`;
        
        if (!canProceed) {
          response += `\n💡 Next Steps:\n`;
          
          if (args.operation === 'create_stage' && !parent) {
            response += `1. Run: find_or_create_project("Your Project Name")\n`;
            response += `2. Then use the returned ID with create_stage\n`;
          } else if (args.operation === 'create_task' && parent && parent.level === 'master') {
            response += `1. First create a stage: create_stage(project_id: "${parent.id}", title: "Stage Name")\n`;
            response += `2. Then create tasks under that stage\n`;
          } else if (args.operation === 'create_subtask' && parent && parent.level !== 'task') {
            response += `1. You need to create or find a task first\n`;
            response += `2. Subtasks can only be added to task-level items\n`;
          }
        } else {
          response += `\n✅ Ready to proceed with ${args.operation}`;
        }
      ...
      try {
        ...
        if (existing && existing.length > 0) {
          ...
          return {
            content: [{
              type: 'text',
              text: `✅ Found existing project: ${project.title}\n` +
                    `ID: ${project.id}\n` +
                    `Project: ${project.project}\n` +
                    `Status: ${project.status}\n` +
                    `Created: ${new Date(project.created_at).toLocaleDateString()}\n` +
                    `Path: ${project.path}\n\n` +
                    `ℹ️ Use this ID for create_stage or create_hierarchical_task operations`
            }],
            isError: false
          };
        }
        ...
        const task = await manager.createTask({
          title: args.title,
          description: args.description || `Project: ${args.title}`,
          level: 'master',
          project: projectSlug,
          priority: args.priority || 'medium',
          status: 'todo',
          tags: args.tags || [],
          metadata: {
            created_via: 'mcp_find_or_create_project'
          }
        });
        ...
        return {
          content: [{
            type: 'text',
            text: `✅ Created new project: ${task.title}\n` +
                  `ID: ${task.id}\n` +
                  `Project: ${task.project}\n` +
                  `Path: ${task.path}\n\n` +
                  `ℹ️ Project didn't exist, so it was created. Use this ID for create_stage or create_hierarchical_task operations`
          }],
          isError: false
        };
      } catch (error) {
        return {
          content: [{
            type: 'text',
            text: `❌ Error in find_or_create_project: ${error.message}`
          }],
          isError: true
        };
      }
