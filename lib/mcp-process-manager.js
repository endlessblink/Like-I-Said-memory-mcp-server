...
export class MCPProcessManager {
  constructor(options = {}) {
    ...
    // Default process patterns - can be overridden by config file
    this.defaultPatterns = {
      'server-markdown.js': { max: 1, priority: 'critical' },  // Only OUR server
      'context7-mcp': { max: 2, priority: 'high' },
      'mcp-server-sequential-thinking': { max: 2, priority: 'high' },
      'mcp-server-playwright': { max: 2, priority: 'medium' },
      'mcp-server': { max: 3, priority: 'medium' },
      // Generic patterns to catch ALL MCP servers
      '-mcp': { max: 2, priority: 'medium' },  // Catches weather-mcp, filesystem-mcp, etc.
      'mcp_': { max: 2, priority: 'medium' },   // Catches mcp_server variants
      '/mcp/': { max: 2, priority: 'medium' }   // Catches paths with /mcp/ in them
    };
    
    // Load custom configuration if available
    this.processPatterns = this.loadConfiguration();
  ...
  loadConfiguration() {
    try {
      const configPath = path.join(process.cwd(), 'data', 'process-manager-config.json');
      if (fs.existsSync(configPath)) {
        const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        this.log('Loaded custom process manager configuration', 'info');
  ...
  /**
   * Get all MCP-related processes - ENHANCED to catch ALL MCP servers
   */
  async getProcesses() {
    try {
      ...
      for (const line of lines) {
        ...
        // ENHANCED: Detect ANY process that looks like an MCP server
        // BUT exclude development tools and non-server processes
        const lowerLine = line.toLowerCase();
        
        // Exclude development tools and build processes
        const isExcluded = (
          lowerLine.includes('vite') ||
          lowerLine.includes('webpack') ||
          lowerLine.includes('rollup') ||
          lowerLine.includes('esbuild') ||
          lowerLine.includes('cross-env') ||
          lowerLine.includes('zellij') ||
          lowerLine.includes('tmux') ||
          lowerLine.includes('npm run') ||
          lowerLine.includes('yarn') ||
          lowerLine.includes('pnpm')
        );
        
        const isMcpLike = !isExcluded && (
          lowerLine.includes('mcp-server') ||
          lowerLine.includes('model-context-protocol') ||
          (lowerLine.includes('node') && (
            lowerLine.includes('mcp-server') ||
            lowerLine.includes('_mcp') && lowerLine.includes('server') ||
            lowerLine.includes('-mcp') && lowerLine.includes('server') ||
            lowerLine.includes('server-mcp') ||
            lowerLine.includes('server_mcp')
          )) ||
          // Specific known MCP patterns
          lowerLine.includes('server-markdown.js') ||
          lowerLine.includes('context7-mcp') ||
          lowerLine.includes('sequential-thinking') ||
          lowerLine.includes('playwright-mcp')
        );
        
        if (isMcpLike && !line.includes('grep')) {
          const parts = line.trim().split(/\s+/);
          if (parts.length >= 11) {
            const pid = parseInt(parts[1]);
            
            // Skip our own process and duplicates
            if (pid === ourPid || processedPids.has(pid)) {
              continue;
            }
            
            processedPids.add(pid);
            
            // Find matching pattern with config
            let matchedPattern = null;
            let config = { max: 2, priority: 'medium' }; // Default for unknown MCPs
            
            for (const [pattern, patternConfig] of Object.entries(this.processPatterns)) {
            ...
            // If no pattern matched but it's MCP-like, use generic pattern
            if (!matchedPattern) {
              matchedPattern = 'unknown-mcp-server';
            }
  ...
  analyzeHealth(processes) {
    ...
    // Check process count limits for all patterns (including unknown)
    for (const [pattern, processes] of Object.entries(processGroups)) {
      const count = processes.length;
      const config = this.processPatterns[pattern] || { max: 2, priority: 'medium' }; // Default for unknown
      ...
      if (count > config.max) {
        ...
        health.actions.push({
          ...
          pattern,
          excess,
          processes: processes.slice(config.max) // Keep most recent
        });
      }
    }
