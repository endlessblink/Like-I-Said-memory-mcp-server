...
export class VectorStorage {
  ...
  async initialize() {
    ...
    try {
      ...
      console.error('Vector storage initialized successfully with local embeddings');
    } catch (error) {
  ...
  async rebuildIndex(memories, tasks) {
    ...
    // Use stderr for logging to avoid breaking JSON-RPC protocol
    console.error(`Rebuilt vector index with ${memories.length} memories and ${tasks.length} tasks`);
  }
}