 ...
 * WSL2 automatically uses sql.js for better compatibility
 */
export class DatabaseAdapter {
  ...
  detectWSL2() {
    try {
      ...
      if (isWSL) {
        console.log('[DatabaseAdapter] WSL2 environment detected - will use sql.js for compatibility');
      }
      
      return isWSL;
    } catch (error) {
      // If detection fails, assume not WSL2
      return false;
    }
  }
  ...
  async initialize() {
    ...
    const forceDb = process.env.LIKE_I_SAID_DB || process.env.USE_SQLJS || process.env.LIKE_I_SAID_WSL2_MODE;
    
    // Force sql.js for WSL2 or if explicitly requested
    if (this.isWSL2 || forceDb === 'sqljs' || forceDb === 'true') {
      const reason = this.isWSL2 ? 'WSL2 environment' : 'environment variable';
      console.log(`[DatabaseAdapter] Using sql.js (${reason})`);
