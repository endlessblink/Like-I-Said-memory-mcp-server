  ...
  static async applyAutomatedUpdate(taskId, automationResult, taskStorage) {
    ...
    try {
      ...
      if (timeSinceAutomation > 60 * 60 * 1000) {
        console.error(`Automation result too old for task ${taskId}, skipping`);
        return false;
      }
      ...
      console.error(`✅ Automated status change applied: ${task.serial} → ${automationResult.newStatus} (${automationResult.reason})`);
