...
export async function validateServerResponse(port, timeout = 5000) {
  ...
  try {
    console.log(`   → Testing simple endpoint: http://localhost:${port}/test`);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`   → URL attempted: http://localhost:${port}/api/status`);
...
export async function startServerWithValidation(server, preferredPort = 3001) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(availablePort, '0.0.0.0', async (err) => {
      ...
      console.log(`🚀 Server listening on port ${actualPort}, validating response...`);
      
      // Longer delay to ensure all Express routes are fully registered
      // Some systems need more time for middleware initialization
      ...
      if (isResponding) {
        ...
        console.log(`✅ Server startup validated successfully on port ${actualPort}`);
