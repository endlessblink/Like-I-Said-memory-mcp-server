...
function getServerHost() {
  ...
  // Development: Use explicit IPv4 binding for cross-platform compatibility
  // 0.0.0.0 = bind to all IPv4 addresses
  // This works reliably on Windows, macOS, Linux, and WSL2
  // While dual-stack (undefined) would support IPv6, it causes issues in WSL2
  // and IPv4-only is sufficient for local development
  return '0.0.0.0';
}
...
export async function startServerWithValidation(server, preferredPort = 3001) {
  ...
  safeConsole.log(`🌐 Server binding mode: ${host === '0.0.0.0' ? 'IPv4-only' : host ? `specific host ${host}` : 'dual-stack'} on port ${availablePort}`);
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleServerError = (err) => {
      ...
      if (err.code === 'EADDRINUSE') {
        ...
        // Fallback to explicit IPv4 binding
        server.listen(availablePort, '0.0.0.0', (err2) => {
          if (err2) {
            reject(new Error(`❌ Cannot bind to any address on port ${availablePort} - ${err2.message}`));
          }
        });
      } else if (err.code === 'EACCES') {
      ...
      server.listen(availablePort, bindHost, async (err) => {
        ...
        if (isResponding) {
          ...
          if (actualHost === '0.0.0.0') {
            safeConsole.log(`✅ Server listening on all IPv4 interfaces (0.0.0.0:${actualPort})`);
            safeConsole.log(`📍 Available at:`);
            safeConsole.log(`   - http://localhost:${actualPort}`);
            safeConsole.log(`   - http://127.0.0.1:${actualPort}`);
          } else if (actualHost === '::') {
            safeConsole.log(`✅ Server listening on all interfaces (IPv6 dual-stack)`);
            safeConsole.log(`📍 Available at:`);
            safeConsole.log(`   - http://localhost:${actualPort}`);
            safeConsole.log(`   - http://127.0.0.1:${actualPort}`);
            safeConsole.log(`   - http://[::1]:${actualPort}`);
          } else {
