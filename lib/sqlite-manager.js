...
export class SQLiteManager {
  ...
  async initialize() {
    try {
      ...
      console.log(`[SQLiteManager] Initialized with ${this.dbType} database`);
      
      // Enable WAL mode for concurrent access (only for real SQLite, not in WSL2)
      if (this.dbType === 'better-sqlite3' && !process.env.LIKE_I_SAID_DISABLE_WAL) {
        try {
          this.db.pragma('journal_mode = WAL');
          this.db.pragma('synchronous = NORMAL');
          this.db.pragma('cache_size = 1000000');
        } catch (error) {
          console.log('[SQLiteManager] WAL mode setup failed, continuing without WAL:', error.message);
        }
      } else if (this.dbType === 'sql.js') {
        // sql.js doesn't support WAL mode, but we can set other pragmas
        try {
          this.db.pragma('synchronous = NORMAL');
          this.db.pragma('cache_size = 1000000');
        } catch (error) {
          // Ignore pragma errors for sql.js
        }
