 ...
 * Uses WAL mode for concurrent access and performance
 */
export class SQLiteManager {
  constructor(dataDir = null) {
    this.dataDir = dataDir || path.join(__dirname, '..', 'data');
    this.dbPath = path.join(this.dataDir, 'tasks-v3.db');
    this.db = null;
  ...
  async initialize() {
    try {
      ...
      console.log('[SQLiteManager] Database initialized with WAL mode');
