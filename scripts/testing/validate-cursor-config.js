...
/**
 * Validates Cursor IDE configuration format
 */
...
function validateCursorConfig(configPath) {
  console.log(`Validating Cursor IDE config: ${configPath}`);
  ...
  try {
    ...
    // Validate structure (Cursor uses same format as Claude Desktop)
    ...
    if (serverConfig.env && serverConfig.env.MCP_QUIET !== 'true') {
      console.warn('⚠️  Cursor works best with MCP_QUIET=true');
    }
    
    console.log('✅ Cursor IDE configuration is valid');
...
if (!configPath) {
  console.error('Usage: node validate-cursor-config.js <config-path>');
  process.exit(1);
}

validateCursorConfig(configPath);