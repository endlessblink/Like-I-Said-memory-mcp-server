  ...
  try {
    ...
    switch (name) {
      ...
      case 'search_memories': {
        ...
        // Expand query using intelligence system
        ...
        try {
          ...
          const vectorResults = await vectorStorage.searchSimilar(query, 'memory', 15);
        ...
        combinedResults = queryIntelligence.rankResults(combinedResults, query, expandedQueries);
        ...
        const resultList = combinedResults.slice(0, 20).map((memory) => {
          ...
          const scoreIcon = memory.relevanceScore > 15 ? '⭐' : '';
          ...
          return `${matchIcon}${scoreIcon} ${memory.id} | 📝 ${preview}${matchInfo} | 🏷️ ${memory.tags?.join(', ') || 'no tags'} | 📁 ${memory.project || 'default'}`;
        }).join('\n');
        ...
        return {
          content: [
            {
              type: 'text',
              text: `🔍 Found ${combinedResults.length} memories matching "${query}"${project ? ` in project: ${project}` : ''}:\n\n${resultList}${combinedResults.length > 20 ? `\n\n... and ${combinedResults.length - 20} more results` : ''}\n\n📊 Results: ${matchCounts.exact} exact 🎯, ${matchCounts.expanded} expanded 🔤, ${matchCounts.semantic} semantic 🧠\n💡 Search intent: ${queryIntent.type}${expandedQueries.length > 1 ? `\n🔄 Also searched: ${expandedQueries.slice(1, 4).join(', ')}` : ''}`,
            },
          ],
        };
      }
