#!/usr/bin/env python3
"""
Build a JSON-RPC 2.0 compliant MCP DXT based on Zod union validation analysis
This addresses the specific Zod schema validation issues identified
"""
...
class JSONRPCCompliantDXTBuilder:
    def __init__(self):
        self.build_dir = Path("dxt-jsonrpc-build")
        
    def clean_build_dir(self):
        if self.build_dir.exists():
            shutil.rmtree(self.build_dir)
        self.build_dir.mkdir()
        print("✓ Created JSON-RPC compliant build directory")
        
    def create_server_directory(self):
        server_dir = self.build_dir / "server"
        server_dir.mkdir()
        
        # Create JSON-RPC 2.0 compliant server
...
''')
        ...
        print("✓ Created JSON-RPC 2.0 compliant MCP server")
    ...
    def create_manifest(self):
        ...
        manifest = {
            ...
            "name": "like-i-said-v2",
            "version": "1.0.0",
            "description": "JSON-RPC 2.0 compliant memory management for Claude Desktop",
            ...
            "tools": [
                {
                    "name": "test_connection",
                    "description": "Test MCP connection with JSON-RPC 2.0 compliance verification"
                },
                {
                    "name": "add_memory",
                    "description": "Store memory with strict input validation"
                },
                {
                    "name": "list_memories",
                    "description": "List stored memories with limit controls"
                }
            ]
        }
        
        (self.build_dir / "manifest.json").write_text(json.dumps(manifest, indent=2))
        print("✓ Created JSON-RPC compliant manifest")
        
    def create_readme(self):
        """Create comprehensive README"""
        readme = self.build_dir / "README.md"
        readme.write_text("""# Like-I-Said v2 - JSON-RPC 2.0 Compliant
...
""")
    ...
    def build_dxt(self):
        """Build the JSON-RPC compliant DXT"""
        dxt_filename = "like-i-said-v2-jsonrpc.dxt"
    ...
    def build(self):
        """Build the JSON-RPC compliant DXT"""
        print("Building JSON-RPC 2.0 Compliant Like-I-Said v2 DXT...")
        print("=" * 55)
        ...
        print("\\n" + "=" * 55)
        print("✅ JSON-RPC 2.0 COMPLIANT DXT READY!")
        print("\\nSpecifically addresses Zod union validation errors:")
        print("- Proper ID field handling (string/number/null)")
        print("- Strict JSON-RPC 2.0 message format compliance")
        print("- Proper error response structure")
        print("- Required field validation")
        print("- Type-safe parameter handling")
        print("\\nThis should eliminate ALL Zod validation errors!")
        
        shutil.rmtree(self.build_dir)
        print("✓ Build directory cleaned")

if __name__ == "__main__":
    builder = JSONRPCCompliantDXTBuilder()
    builder.build()