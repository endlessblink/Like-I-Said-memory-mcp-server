#!/usr/bin/env python3
"""
Build comprehensive Like-I-Said DXT with all 23 tools
This creates a self-contained DXT package with embedded Python and all dependencies
"""
...
def create_comprehensive_dxt():
    """Create comprehensive DXT with all 23 tools"""
    
    # Create temporary directory for DXT contents
    dxt_dir = Path("dxt_build_comprehensive")
    if dxt_dir.exists():
        shutil.rmtree(dxt_dir)
    dxt_dir.mkdir()
    
    print("Creating comprehensive Like-I-Said DXT...")
    ...
    # Create main server runner that ensures proper JSON-RPC compliance
...
# Check virtual environment requirement
if not hasattr(sys, 'real_prefix') and not (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
    if os.environ.get('FORCE_NO_VENV') != 'true':
        print("ERROR: Like-I-Said must run from a virtual environment", file=sys.stderr)
        print("INFO: Running in embedded Python, setting FORCE_NO_VENV=true", file=sys.stderr)
...
'''
    ...
    # Create manifest.json (not dxt.json!) with proper structure
    manifest = {
        "version": "1",
        "schemaVersion": "0.1.0",
        "name": "like-i-said-v2",
        "displayName": "Like-I-Said Memory v2 (Python)",
        ...
        "license": "MIT",
        "keywords": ["memory", "mcp", "ai", "assistant", "tasks", "python"],
        "categories": ["productivity", "ai-tools", "memory-management"],
        "mcpServers": {
            "like-i-said-v2": {
                "command": "python",
                "args": ["server.py"],
                "description": "Like-I-Said Memory Management System v2 with all 23 tools",
                "tools": [
                    {
                        "name": "test_tool",
...
'''
    ...
    # Create the DXT file
    dxt_filename = "like-i-said-v2-comprehensive-python.dxt"
...
if __name__ == "__main__":
    try:
        dxt_file = create_comprehensive_dxt()
